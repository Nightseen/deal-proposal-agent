...
## Feature obligatorio: disclosure "Ver más" en Alcance (solo mobile)

**Toda propuesta creada a partir de esta plantilla debe conservar este feature intacto.** Ya está implementado en `plantilla_propuestas.html` (`initScopeToggle()` en el `<script>` del final del `<body>`, más el botón `<button class="scope-grid__toggle" id="scope-grid-toggle" hidden>` justo después de `#scope-grid`); al duplicar la plantilla no se toca.

Cómo funciona:
1. Alcance admite 4–8 tarjetas (ver feature de íconos arriba), pero mostrar más de 4 de una sentada excede el límite de carga cognitiva (≤4 ítems por grupo) que el resto de la plantilla respeta. `initScopeToggle()` cuenta las `.card` dentro de `#scope-grid`; si hay más de 4, oculta la 5ª en adelante (`.scope-card--collapsed`) y muestra el botón "Ver N más", que al hacer clic revela el resto.
2. Este disclosure es **solo para mobile** (`≤640px`). En desktop/tablet (`min-width: 641px`) una regla CSS fuerza `.scope-card--collapsed{ display: block }` y oculta el botón (`.scope-grid__toggle{ display: none }`), así que siempre se ven todas las tarjetas. Motivo: `.scope-grid` usa `repeat(auto-fit, minmax(240px, 1fr))`, así que en desktop 4 tarjetas visibles suelen quedar 3 en una fila + 1 sola en la fila siguiente — el botón ahí solo completaría esa misma fila con las 2 restantes, sin ahorrar espacio real. En mobile (una sola columna) sí tiene sentido acortar la lista inicial.
3. Funciona sobre cualquier cantidad de tarjetas, generadas por `build-proposal.js` o pegadas a mano por la IA — no depende de cómo se generó `[LISTA_ALCANCE]`.
4. Con 4 tarjetas o menos, `initScopeToggle()` no hace nada y el botón se queda con el atributo `hidden`. Ese atributo por sí solo no basta: la regla base `.scope-grid__toggle{ display: flex; ... }` tiene la misma especificidad que `[hidden]` y, al venir del CSS de autor, le gana a la hoja de estilos del navegador — el botón se veía igual sin acción alguna. Por eso hay una regla `.scope-grid__toggle[hidden]{ display: none; }` justo después de la base, con más especificidad, que sí lo oculta cuando no hay nada que expandir. Si se agrega alguna otra regla de `display` para este botón, debe quedar después de esta o repetirla.

## Feature obligatorio: sección 08 estática ("Resultados comprobados")

**Toda propuesta creada a partir de esta plantilla debe conservar este bloque tal cual.** La sección 08 ("Por qué confiar en nosotros") ya no toma datos del cliente — es un caso de uso real fijo, con 5 cifras mostrando: ~617 citas/mes automatizadas, 70% menos carga en recepción, ~140 citas recuperadas/mes, ~$14M retorno por recuperación/mes, <5% tasa de cancelación/ausentismo. Reutiliza el mismo marcado que la sección 04 (`.card`, `.scope-card__title`, `.scope-card__desc`, `.card--icon` / `.card__icon`), pero **no** como 5 tarjetas individuales: `.results-grid` las funde en una sola franja continua (fondo blanco, borde y radio compartidos) dividida por líneas finas (`border-left`/`border-top` según breakpoint), con el ícono en línea (no en placa flotante como en Alcance). Este tratamiento es deliberado: una revisión de diseño (`$impeccable critique`, 2026-07-16) y su detector de patrones coincidieron, de forma independiente, en que 5 tarjetas idénticas con ícono flotante leían como el "hero-metric template" que `DESIGN.md` prohíbe, solo reubicado fuera del hero. Al duplicar la plantilla no se toca ni se le pide contenido al usuario para esta sección.

Las 5 cifras comparten el mismo tratamiento visual (mismo tamaño, mismo color de texto). Se probó destacar la primera (~617, la más alta) en azul de marca y a mayor tamaño para romper la simetría, pero se revirtió (2026-07-16, feedback directo del usuario) porque en la práctica leía como una inconsistencia — una tarjeta desentonando del resto — y no como una ruptura deliberada de patrón. Si se vuelve a intentar destacar una cifra, debe ser un tratamiento que no rompa la coherencia visual de la franja.
...
- Título por defecto del hero: **"Propuesta comercial - Agente IA para [NOMBRE_CLIENTE]"** — solo `[NOMBRE_CLIENTE]` es variable. (Antes decía "Implementación agentes IA"; se acortó a "Agente IA" por feedback directo del usuario, 2026-07-16.)
- Subtítulo del hero: **"[NOMBRE_CLIENTE] + ClaryTree"** (ya no usa `[NOMBRE_PROYECTO]`, que se eliminó de la plantilla).
- CTA del hero (`.hero__cta`, texto "Ver propuesta completa"): enlaza a `#entendimiento` (la sección 02, justo debajo del hero), no a `#cierre`. Es un ancla de navegación dentro de la página, distinta del CTA de WhatsApp de la sección 09 — no lleva al cliente directo al cierre/precio antes de leer el resto de la propuesta.
